angular.module('angularApp')
    .controller('productsctrl', function($scope, $rootScope, $http, _) {
        $rootScope.nav = true;
        $scope.my = 'Test'
        $scope.gridOptions = {}
        $scope.searchKeyword = '';
        ...
        $scope.allStyles = function() {
            $http.get("bottleData/allStyles.json")
                .then(function(data) {
                    $scope.currentPage = 0
                    $scope.data = data.data
                    $scope.listName = "All Styles"
                    var arrToJsonStyle = [];
                    var arrToJsonSize = [];
                    var arrToJsonNeck = [];
                    ...
                    var Neck = _.map($scope.data, function(num) { return num.neckFinish });
                    var uniqNeck = _.uniq(Neck);
                    _.each(uniqNeck, function(num) {
                        var jsons = { neckFinish: num }
                        arrToJsonNeck.push(jsons)
                    })
                    $scope.arrToJsonNeck = arrToJsonNeck;
        ...
        $scope.searchSpecs = function() {
            ...
            $scope.searchField.neck = $scope.searchField.neck || null;
            ...
            $http.get("bottleData/allStyles.json")
                .then(function(data) {
                    ...
                    var neck = $scope.searchField.neck == null ? null : $scope.searchField.neck.neckFinish;
                    var material = $scope.searchField.material == null ? null : $scope.searchField.material.material;
                    var specsArray = [];
                    specsArray.push(style, size, neck, material)
                    ...
                    if (_.isEmpty(selected)) {
                        $scope.data = "";
                        $scope.noData = "No Such Bottle Found"
                    } else {
                        $scope.data = selected;
                        $scope.noData = "";
                    }
